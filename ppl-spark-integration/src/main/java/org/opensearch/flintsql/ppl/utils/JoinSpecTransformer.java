...
public interface JoinSpecTransformer {
    ...
    static LogicalPlan join(LogicalPlan left, LogicalPlan right, org.opensearch.flintsql.ast.tree.Join.JoinType joinType, Optional<Expression> joinCondition, org.opensearch.flintsql.ast.tree.Join.JoinHint joinHint) {
        return new Join(left, right, getType(joinType), Option.apply(joinCondition.orElse(null)), JoinHint.NONE());
    }

    static JoinType getType(org.opensearch.flintsql.ast.tree.Join.JoinType joinType) {
        switch (joinType) {
            case INNER:
                return Inner$.MODULE$;
            case LEFT:
